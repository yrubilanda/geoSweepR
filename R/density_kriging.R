...
#' @param crs The Coordinate Reference System associated with your data's location (e.g., 4326).
...
#' density_kriging(my_data, lon_col = "longitude", lat_col = "latitude", crs = 4236,
...
density_kriging  <- function(data, lon_col, lat_col, crs, col_data = NULL, filter_by = NULL, grid_size = 500, variogram_model = "Sph") {
  #check if there was a filter parameter set
  if (!is.null(filter_by) && !is.null(col_data)) {
    data <- dplyr::filter(data, .data[[col_data]] == filter_by)
  }
   # convert the data into an sf
  d_sf <- sf::st_as_sf(
    data,
